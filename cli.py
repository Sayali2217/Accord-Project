...
def main():
    ...
    print(f"Initializing CrewAI with model: {args.model}")
    ...
    validator = Agent(
        ...
        verbose=True,
        allow_delegation=False, # We use the retry mechanism inside tasks context
        llm=llm
    )
    ...
    # ==========================
    task1 = Task(
        description=f"Analyze the following legal requirement: '{args.input}'. Extract parties, core clauses, variable names, and expected data types. Output a structured JSON or markdown analysis.",
    ...
    # ==========================
    # Implementing the sequential execution task 1 -> task 2 -> task 3
    crew = Crew(
    ...
    try:
        # Retry loop for validator failures to simulate "retry on fail"
        max_retries = 3
        attempt = 1
        ...
        while "FAIL" in str(result).upper() and attempt < max_retries:
            ...
            # In a true dynamic system we would pass the validation errors back to the Template Writer manually here
            # For simplicity, we create a specialized retry task
            retry_task = Task(
